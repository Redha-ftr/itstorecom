...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		url						: "<?php echo base_url() . 'subcategory/data'; ?>",
		primaryField			: 'id', 
		sortBy					: 'id',
		orderBy					: 'ASC',
		rowNumber				: true,
		...
		columns					: [
		{field: 'nama', title: '<?=$this->lang->line('title_subkategori')?>', editable: true, sortable: false, width: '', align: 'center', search: true},
	...
	function create_update_form(rowIndex) {
		$.post("<?php echo base_url() . 'subcategory/form'; ?>", {index : rowIndex}).done(function(data) {
	...
	function delete_action(rowIndex) {
		swal({   
			...
			$.post("<?php echo base_url() . 'subcategory/delete'; ?>", {id : row.id}).done(function(data) {
				$('.loading-panel').hide();
				$('.form-panel').hide();

...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		url						: "<?php echo base_url() . 'official_brand/data'; ?>",
		primaryField			: 'id', 
		sortBy					: 'id',
		orderBy					: 'ASC',
		rowNumber				: true,
	...
	function create_update_form(rowIndex) {
		$.post("<?php echo base_url() . 'official_brand/form'; ?>", {index : rowIndex}).done(function(data) {
	...
	function delete_action(rowIndex) {
		swal({   
			...
			$.post("<?php echo base_url() . 'official_brand/delete'; ?>", {id : row.id}).done(function(data) {
				$('.loading-panel').hide();
				$('.form-panel').hide();
